...
class Poly {
  ...
  //  --- Named Constructors
  /// .Decode(str, p)
  Poly.decode({this.encodedString, this.precision}) {
    decodedCoords = _decode(encodedString!, precision!);
    encodedString = encodedString;
  }

  /// .Encode(coords, p)
  Poly.encode({this.decodedCoords, this.precision}) {
    encodedString = encodePoly(decodedCoords!, precision!);
    decodedCoords = decodedCoords;
  }

  // .Distance(str, u)
  Poly.distance({this.encodedString, this.unit}) {
  ...
  List<List<double>> _decode(String str, int precision) {
    int index = 0, lat = 0, lng = 0, shift = 0, result = 0, byte, latitudeChange, longitudeChange, factor = pow(10, precision is int ? precision : 5) as int;
  ...
  /// @param {int} precision
  /// @returns {String}
  String encodePoly(List<List<double>> coordinates, int precision) {
  ...
  double _haversineDistance(List<double> _point1, List<double> _point2) {
    _point1.map((item) => _degToRad(item));
    _point2.map((item) => _degToRad(item));
